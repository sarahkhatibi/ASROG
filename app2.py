    ...
    if uploaded_file is not None:
        ...
        if df is not None:
            ...
            with st.expander("📊 اطلاعات فایل"):
                ...
                st.write(f"**بازه تاریخی:** {df['تاریخ_شمسی'].min()} تا {df['تاریخ_شمسی'].max()}")
        ...
        if st.button("استفاده از داده‌های نمونه", type="secondary"):
            ...
            df = load_and_parse(BytesIO(pd.DataFrame(sample_data).to_csv(index=False).encode('utf-8')))
            df['RiskScore'] = df.apply(calculate_risk_score, axis=1)
            df['RiskFlag'] = df['RiskScore'].apply(get_risk_flag)
            df['RiskLevel'] = df['RiskScore'].apply(get_risk_level)
            st.session_state.df = df
...
if 'df' in st.session_state:
    ...
    with col2:
        ...
        st.metric("🔴 پرریسک", f"{high_risk:,}", 
                 delta=f"{high_risk/total_trans*100:.1f}%" if total_trans > 0 else "0%")
    ...
    with st.expander("فیلترها", expanded=True):
        ...
        with filter_col3:
            min_score, max_score = int(df['RiskScore'].min()), int(df['RiskScore'].max())
    ...
    with tab4:
        ...
        with col2:
            ...
            st.download_button(
                ...
                mime="application/vnd.openxmlformats-officedesign.spreadsheetml.sheet"
            )
        ...
        with summary_col1:
            st.metric("تعداد ترانس‌های پرریسک", 
                     filtered_df[filtered_df['RiskLevel'] == 'پرریسک'].shape[0])
